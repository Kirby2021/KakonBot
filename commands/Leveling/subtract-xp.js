...
module.exports = {
  name: "subtract-xp",
  aliases: ['sxp','stxp','subtxp'],
  timeout: "3000",
  description: "Subtract/Remove XP of a member",
  ...
  if(!data) return message.channel.send(`Leveling System is not enabled`)
    ...
    if(!xp || !Number(xp)) return message.channel.send(`You must specify XP as a number to subtract`)
...
message.channel.send(new MessageEmbed()
  ...
  .setColor("RANDOM")
  .setFooter(`Removed ${xp} XP to ${member.user.tag} || Requested by: ${message.author.tag}`)
)
  }
}