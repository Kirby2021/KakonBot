...
module.exports = {
  ...
  if(!data) return message.channel.send(`Leveling System is not enabled`)
    ...
    if(!xp || !Number(xp)) return message.channel.send(`You must specify Level as a number to set`)
...
message.channel.send(new MessageEmbed()
  ...
  .setColor("RANDOM")
  .setFooter(`Set ${member.user.tag} 's Level(s) to ${xp} || Requested by: ${message.author.tag}`)
)
  }
}