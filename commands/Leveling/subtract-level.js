...
module.exports = {
  name: "subtract-level",
  aliases: ['slv','stlv','subtlv'],
  timeout: "3000",
  description: "Subtract/Remove Level of a member",
  ...
  if(!data) return message.channel.send(`Leveling System is not enabled`)
    ...
    if(!xp || !Number(xp)) return message.channel.send(`You must specify Level as a number to remove`)
 await Levels.subtractLevel(member.id, message.guild.id, xp);
...
message.channel.send(new MessageEmbed()
  ...
  .setColor("RANDOM")
  .setFooter(`Removed ${xp} Level(s) to ${member.user.tag} || Requested by: ${message.author.tag}`)
)
  }
}