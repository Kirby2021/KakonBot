...
module.exports = {
    name: 'verify-all',
    description: 'Verify for all members/bots',
    category: 'setup',
    timeout: '5000',
    aliases: ['va'],
               ...
               message.channel.send(new MessageEmbed()
                ...
                if(emoji === '👥') {
                    ...
                    message.guild.members.cache.filter(u => !u.user.bot).forEach(async (member, id) => {
                        ...
                        .setDescription(`You have been force verified **${message.guild.name}** by **${message.author.tag}**`)
                        .setColor("GREEN")
                    )
                    });
                    message.channel.send(`Success ! All Members was verified on the server`)
                 }
                 if(emoji === '🤖') {
                    msg.delete()
                    message.guild.members.cache.filter(u => u.user.bot).forEach(async (member, id) => {
                    ...
                    message.channel.send(`Success ! All Bots was verified on the server`)
                 }
               })
            } else {
