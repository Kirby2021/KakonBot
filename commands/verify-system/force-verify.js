...
module.exports = {
    name: 'force-verify',
    description: 'Force verify the member',
    category: 'setup',
    timeout: '5000',
    aliases: ['fvv'],
    usage: '[@Member]',
    ...
    run: async(client, message, args) => {
        ...
        Schema.findOne({ Guild : message.guild.id }, async(err, data) => {
            ...
            if(!data) {
               ...
               if(member.roles.cache.get(role.id)) return message.channel.send(`${member.user.tag} has been verified already`)
               message.channel.send(new MessageEmbed()
                    .setTitle(`Confirmation ...`)
                    .setDescription(`**Emoji: ✅ | Force-Verify \n Emoji: ❌ | Cancel the command**`)
                    .setColor("RED")
                ...
                if(emoji === '✅') {
                    ...
                    message.channel.send(`Confirmed. ${member.user.tag} was verified on ${message.guild.name}.`)
                 }
                 if(emoji === '❌') {
                    msg.delete()
