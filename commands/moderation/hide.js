...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(!message.guild.me.hasPermission("MANAGE_CHANNELS","MANAGE_MESSAGES")) return message.channel.send(new MesssageEmbed().setDescription(`**${error} I must be an Admin**`).setColor("RED").setFooter(`Requested by: ${message.author.tag}`,message.author.displayAvatarURL({ dynamic:true })))
        const channel = await message.mentions.channels.first() || message.guild.channels.cache.get(args[0]) || message.channel
        const everyone = message.guild.roles.cache.find(role => role.name == '@everyone')
        if(!channel.permissionsFor(everyone).has(["VIEW_CHANNEL"])) return message.channel.send(new MessageEmbed().setTitle(`Error`).setDescription(`${error} This channel is hiden already`).setColor("RED"))
        await channel.createOverwrite(client.user, {
            VIEW_CHANNEL: true
        },`Kakon Moderation - Hide Command - Action By: ${message.author.tag}`)
        await channel.updateOverwrite(everyone, {
            VIEW_CHANNEL: false
        },`Kakon Moderation - Hide Command - Action By: ${message.author.tag}`)
        channel.send(new MessageEmbed().setDescription(`**${success} ${channel} has been hided.**`).setColor("GREEN"))
        .catch(err => {
            console.log(err)
